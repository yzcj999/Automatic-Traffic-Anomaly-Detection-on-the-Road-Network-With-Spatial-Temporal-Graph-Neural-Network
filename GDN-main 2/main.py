...
class Main():
    def __init__(self, train_config, env_config, debug=False):
        ...
        # 获取我们数据中的存在的节点，实际上就是对于获得每一列的名字
        feature_map = get_feature_map(dataset)
        fc_struc = get_fc_graph_struc(dataset)        # 初始化默认的结构，默认是全联接

        set_device(env_config['device'])
        self.device = get_device()

        fc_edge_index = build_loc_net(fc_struc, list(
            train.columns), feature_map=feature_map)    # 转化为pyg使用的边临界的方式
        fc_edge_index = torch.tensor(fc_edge_index, dtype=torch.long)

        self.feature_map = feature_map

        train_dataset_indata = construct_data(
            train, feature_map, labels=0)                     # 根绝feature_map 进行选择数据
        ...
        train_dataset = TimeDataset(
            train_dataset_indata, fc_edge_index, mode='train', config=cfg)  # 组织为我们需要的数据结构
    ...
    def run(self):
        ...
        # 主要就是这里函数的计算过程没有看懂
        # 当得到测试集和验证集的预测后，不知到如何得到最终的结果
        self.get_score(self.test_result, self.val_result)

    def get_loaders(self, train_dataset, seed, batch, val_ratio=0.1):
        '''
            这里的是将我们的train_Dataset 根据我们的val_ratio 转化为另一部分的验证集
        '''
    ...
    def get_score(self, test_result, val_result):
        ...
        # 这个函数不知到是干什么的
        top1_best_info = get_best_performance_data(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '-slide_stride', help='slide_stride', type=int, default=5)
    parser.add_argument('-save_path_pattern',
                        help='save path pattern', type=str, default='')
    parser.add_argument('-dataset', help='wadi / swat',
                        type=str, default='wadi')
    ...
    parser.add_argument('-out_layer_inter_dim',
                        help='out_layer_inter_dim', type=int, default=256)
    ...
    parser.add_argument('-load_model_path',
                        help='trained model path', type=str, default='')
